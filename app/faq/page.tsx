...
export default function FAQPage() {
  ...
  const openContentPage = (itemId: string) => {
    router.push(`/faq/${itemId}`)
  }

  return (
    <>
      {/* Hero Section */}
      <section className="bg-gradient-to-r from-primary to-primary-dark py-12 md:py-20">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center">
            <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">{t.hero?.title || "คำถามที่พบบ่อย"}</h1>
            <p className="text-lg text-white/80 mb-8">
              {t.hero?.subtitle || "ค้นหาคำตอบสำหรับคำถามที่พบบ่อยเกี่ยวกับบริการของเรา"}
            </p>
            <div className="relative max-w-2xl mx-auto">
              <Input
                type="search"
                placeholder={t.hero?.searchPlaceholder || "ค้นหาคำถามที่คุณสงสัย"}
                className="pl-12 bg-white text-gray-900 rounded-full shadow-lg"
                value={searchQuery}
                      ...
                      <AccordionContent className="px-4 pb-4">
                        <div className="prose max-w-none">
                          ...
                          <Button
                            ...
                            onClick={() => openContentPage(item.id)}
                          >
                            {t.readMore || "อ่านเพิ่มเติม"}
                          </Button>
                        </div>
                      </AccordionContent>
                    </AccordionItem>
